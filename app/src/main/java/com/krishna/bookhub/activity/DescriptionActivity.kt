...
class DescriptionActivity : AppCompatActivity() {
                ...
                object : JsonObjectRequest(Request.Method.POST, url, jsonParams, Response.Listener {
                    ...
                    try {
                        ...
                        if (success) {
                            ...
                            val checkFav = DBAsycTask(applicationContext, bookEntity, 1).execute()
                            ...
                            btnAddToFav.setOnClickListener {
                                if (!DBAsycTask(applicationContext, bookEntity, 1).execute().get()){

                                    val asyc = DBAsycTask(applicationContext, bookEntity, 2).execute()
                                    val result = asyc.get()
                                    if(result){
                                    ...
                                    val asyc = DBAsycTask(applicationContext, bookEntity, 3).execute()
                                    val result = asyc.get()
                                    if(result){
    ...
    class DBAsycTask (val context: Context, val bookEntity: BookEntity, val mode: Int) :
        AsyncTask<Void, Void, Boolean>() {
        val db = Room.databaseBuilder(context, BookDatabase::class.java, "books-db").build()

        /*
        Mode1 -> check DB if book is favourite or not
        Mode2 -> save the book in DB as favourites
        Mode 3 -> remove the fovourite book
        */
        override fun doInBackground(vararg params: Void?): Boolean {

            when (mode) {
                1 -> {
                    //check if the book is favourites or not
